...
var color=["#37458b","#de1615","#0b8543","#991c54","#b11e23","#057e7c","#5b2379","#308cc6","#808080","#191717"];
	vue=new Vue({
		...
		computed: {
			...
			yColumn:{
			  	get: function () {
			      return this.yColumn_sel;
			    },
			    set: function (newValue) {
			    	if(!newValue) return;
			    	this.yColumn_sel = newValue;
			        $("#yColumn").selectpicker("val",newValue);
			    }
			}
		},
		watch:{
			input:function(val,oldVal){
				$.ajax({
					url: 'public/draw/json/pictorialBarFileData.json',  
					type:'get',
					data:{
						fileName:val
					},
	...
	$("#use_default").click(function(){
		$.ajax({
			url: 'public/draw/json/pictorialBar.json',  
			type:'get',
			data:tool_id,
			dataType: "json",
			success:function(data) {
	...
	$("#submit_paras").click(function(){
		...
		if(formData.input==""){
			...
			$.ajax({
				...
				success:function(data) {
					myChart.hideLoading();
					updateEchartsData(myChart,formData,data["content"],vue.xColumn,vue.yColumn,vue.yMaxNum);
				},    
				error : function(XMLHttpRequest) {
					alert(XMLHttpRequest.status +' '+ XMLHttpRequest.statusText);
				}
...
function updateEchartsData(echarts,echartsStyle,echartsData,xColumn,yColumn,yMaxNum){
	var bodyMax=yMaxNum;
