...
$(function() {
	vue = new Vue({
		...
		computed: {
			canSelectFields:{
				get:function(){	
					var selectFields = this.fileData.content[0];
					if(!selectFields) {
						return [];
					}
					debugger
					var result = selectFields.slice(); 
					var placeName = this.placeName;
					var latitude = this.latitude;
					var longitude = this.longitude;
					if(placeName) {
						result.splice(result.indexOf(placeName), 1);
					}
					if(latitude) {
						result.splice(result.indexOf(latitude), 1);
					}
					if(longitude) {
						result.splice(result.indexOf(longitude), 1);
					}
					return result;
				}
			},
			...
			longitude: {
				cache: false,
				get: function() {
					if($("#longitude").data('selectpicker'))
						return $("#longitude").selectpicker("val");
					else 
						return null;
				},
				set: function(newValue) {
					if(!newValue) return;
					$("#longitude").selectpicker("val", newValue);
				}
			},
			latitude: {
				cache: false,
				get: function() {
					if($("#latitude").data('selectpicker'))
						return $("#latitude").selectpicker("val");
					else 
						return null;
				},
				set: function(newValue) {
					if(!newValue) return;
					$("#latitude").selectpicker("val", newValue);
				}
			},
			placeName: {
				cache: false,
				get: function() {
					if($("#placeName").data('selectpicker'))
						return $("#placeName").selectpicker("val");
					else 
						return null;
				},
				set: function(newValue) {
					if(!newValue) return;
					$("#placeName").selectpicker("val", newValue);
				}
			},
			title_size: {
				cache: false,
		...
		watch: {
			...
			fileData: function(val, oldVal) {
				this.$nextTick(function() {
					$('#longitude').selectpicker('refresh');
					$('#latitude').selectpicker('refresh');
					$('#placeName').selectpicker('refresh');
					$(".spectrum").spectrum({
	...
	$("#submit_paras").click(function() {
		...
		if(formData.input == "") {
			...
			$.ajax({
				...
				success: function(data) {
					myChart.hideLoading();
					updateEchartsData(myChart, formData, data["content"], vue.longitude, vue.latitude, vue.placeName);
				},
				error: function(XMLHttpRequest) {
					alert(XMLHttpRequest.status + ' ' + XMLHttpRequest.statusText);
				}
...
function updateEchartsData(echartsInstance, echartsStyle, echartsData, longitudeField, latitude) {
	var dcolor = [];
